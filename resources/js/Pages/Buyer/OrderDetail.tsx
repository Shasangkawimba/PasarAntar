...
export default function OrderDetail({ order }: { order: Order }) {
    ...
    return (
        <AuthenticatedLayout>
            <Head title={`Pesanan ${orderState.order_number}`} />

            {/* Header */}
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 md:mb-8">
                <div>
                    <div className="flex items-center gap-2 mb-2">
                        <Link href={route('orders.index')} className="text-[var(--pa-text-muted)] hover:text-[var(--pa-primary)] transition-colors flex items-center gap-1">
                            <span className="material-symbols-outlined" style={{ fontSize: 16 }}>arrow_back</span>
                            <span className="pa-body-sm font-semibold">Kembali ke Daftar Pesanan</span>
                        </Link>
                    </div>
                    <div className="flex flex-wrap items-center gap-3">
                        <h1 className="pa-headline-lg pa-headline-lg-mobile md:pa-headline-lg" style={{ color: 'var(--pa-text-main)' }}>Detail Pesanan</h1>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 md:gap-8">
                ...
                <div className="lg:col-span-8 flex flex-col gap-5 md:gap-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-8">
                        <section className="pa-bento-card-static p-6 flex flex-col justify-between">
                            ...
                            <div className="mt-6 pt-4 flex flex-col gap-1" style={{ borderTop: '1px solid var(--pa-surface-variant)' }}>
                                ...
                                <span className="pa-body-sm" style={{ fontWeight: 600 }}>{formatDate(orderState.created_at)}</span>
                            </div>
                        </section>
                            ...
                            <div>
                                ...
                                {orderState.joki ? (
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="pa-body-sm italic" style={{ color: 'var(--pa-text-muted)' }}>Mencari Joki terdekat...</span>
                                    </div>
                                )}
                            </div>
                            {orderState.joki?.phone_number && (
                                <div className="mt-6 pt-4" style={{ borderTop: '1px solid var(--pa-surface-variant)' }}>
                                    <a href={`tel:${orderState.joki.phone_number}`} className="pa-btn pa-btn-secondary pa-btn-sm w-full justify-center">
                    ...
                    <section className="pa-bento-card-static p-6 md:p-8">
                        <h3 className="pa-headline-md mb-6 flex items-center gap-2">
                            ...
                            Daftar Belanjaan ({orderState.items.length} Barang)
                        </h3>
                        <div className="flex flex-col gap-3">
                            {orderState.items.map((item) => (
                                <div key={item.id} className="flex items-start justify-between p-4 rounded-xl" style={{ backgroundColor: 'var(--pa-surface)', border: '1px solid var(--pa-surface-variant)' }}>
                                    ...
                                    <div className="pa-mono pa-headline-md shrink-0 py-1 px-3 rounded-lg" style={{ backgroundColor: 'var(--pa-primary-fixed)', color: 'var(--pa-primary)' }}>
                                        {item.quantity}x
                                    </div>
                                </div>
                            ))}
                ...
                <div className="lg:col-span-4 flex flex-col gap-5 md:gap-8">
                    ...
                    <section className="pa-bento-card-static p-6">
                        <h3 className="pa-headline-md mb-6 pb-4" style={{ borderBottom: '1px solid var(--pa-surface-variant)' }}>Rincian Biaya</h3>
                            ...
                            <div className="rounded-xl overflow-hidden" style={{ border: '1px solid var(--pa-surface-variant)' }}>
                                {orderState.refund_amount !== null && orderState.refund_amount > 0 ? (
                                    <div className="p-4" style={{ backgroundColor: 'rgba(16,185,129,0.1)' }}>
                                        <span className="pa-label-caps block mb-1" style={{ color: 'var(--pa-status-completed)' }}>Sisa Kembalian (Refund)</span>
                                        <span className="pa-mono pa-headline-lg" style={{ color: 'var(--pa-status-completed)' }}>{formatRupiah(orderState.refund_amount)}</span>
                                        <p className="pa-body-sm mt-2" style={{ color: 'var(--pa-status-completed)', opacity: 0.8 }}>Dana akan dikembalikan oleh Joki.</p>
                                    </div>
                                    ...
                                    <div className="p-4" style={{ backgroundColor: 'rgba(225,29,72,0.1)' }}>
                                        <span className="pa-label-caps block mb-1" style={{ color: 'var(--pa-alert-rose)' }}>Kekurangan Bayar</span>
                                        <span className="pa-mono pa-headline-lg" style={{ color: 'var(--pa-alert-rose)' }}>{formatRupiah(orderState.additional_payment)}</span>
                                        <p className="pa-body-sm mt-2" style={{ color: 'var(--pa-alert-rose)', opacity: 0.8 }}>Bayarkan kekurangan ini ke Joki.</p>
                                    </div>
                                ) : (
                                    <div className="p-4" style={{ backgroundColor: 'var(--pa-surface-container)' }}>
                                        ...
                                        <p className="pa-body-sm mt-2" style={{ color: 'var(--pa-text-muted)' }}>Dana terpakai pas dengan estimasi.</p>
                                    </div>
                                )}
                            </div>
                        )}
                                    ...
                                    <div key={receipt.id} className="rounded-xl overflow-hidden cursor-zoom-in relative group" style={{ border: '1px solid var(--pa-surface-variant)' }} onClick={() => setZoomedImage(receipt.image_url)}>
                                        ...
                                        {receipt.uploader && (
                                            <div className="absolute bottom-0 left-0 right-0 p-2 bg-black/60 text-white pa-label-caps text-xs">
                                                Diupload oleh: {receipt.uploader.name}
                                            </div>
                                        )}
                                    </div>
                                ))}
